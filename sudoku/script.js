...
let mistake = 0; // Initialize mistake count to 0
let hintNumber = 3; // Initialize hint count to 3
...
function onNumberClick() {
  ...
  if (isCorrect) {
    ...
    if (filledSquares.length == 81) endGame("Bravo! Sudoku Conquered!", "green");
    return;
  }
  mistake++; // Increment mistake count
  mistakeElement.innerHTML = mistake + "/3";
...
hintButton.addEventListener("click", function () {
  ...
  activeSquare.classList.add("true");
  hintNumber--; // Decrement hint count
...
function showAlert(message, color) {
  ...
  setTimeout(function () {
    ...
    if (message.includes("Game Over")) {
      newGameButton.click(); // Trigger new game button click
    }
  }, 3000);
}
